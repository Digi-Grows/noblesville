    ...
    await sql_conn.query(fetch_doc, async (err, doc) => {
        ...
        await sql_conn.query(`SELECT * FROM REGISTERED_PRESCRIBER WHERE ID = '${prescriber_id}'`, async (err, prescriber_result) => {
            ...
            docx.on('finalize', (written) => {
                ...
                function sendFax() {
                    ...
                    interfax.outbound.deliver({
                        ...
                        .then(fax => {
                            res.send("FAX Sent to Noblesville Pharmacy  FAX : " + process.env.FAX_NUMBER);
                            console.log("FAX Sent Successfully to Pharmacy FAX : " + process.env.FAX_NUMBER);
                            console.log(doc_file);
                        })
                        .catch(error => {
                            console.error("Error sending fax to : " + process.env.FAX_NUMBER);
                            res.json({err_fax : ` FAX not sent to Noblesville Pharmacy FAX :  ${process.env.FAX_NUMBER}`});
                        });
                }
            ...
            const table3 = [
                ...
                [`SIGNATURE: ${prescriber_result[0].FIRST_NAME} ${prescriber_result[0].LAST_NAME}`, `DATE: ${utc}`],
            ];
            const tableStyle3 = {
                tableColWidth: 8261, tableAlign: "left", tableFontFamily: "Serif", sz: 18
            };
